...
import { ArrowBigDownDashIcon,ChevronDown, X,Menu, User, User2Icon, UserCheck2, Database, SubscriptIcon, BookIcon, SearchX, LoaderPinwheelIcon, BugPlayIcon, ShoppingCart, FileEditIcon, TimerIcon, LucideNewspaper, GitCompareIcon, Home } from 'lucide-react';
...
const mainMenu=(
  ...
  <NavBarMenuModal closeMenuButton={closeMenuButton} handleCloseMenuButton={handleCloseMenuButton}>
    <div className='flex flex-col gap-2 border rounded-lg border-gray-400 p-1
    bg-slate-950/20 backdrop-blur-lg justify-left absolute right-0 top-18 w-34 h-34 justify-center items-center text-green-400 sm:mb-8 animate-in slide-in-from-top duration-700 delay-200 '>

    <Link to='/signup'>
  <p className='text-base border-b px-2 py-1 border-gray-400 hover:text-white'>Sign/up</p>
    </Link>
    <Link to='login'>
  <p className=' text-base border-b px-2 py-1 border-gray-400 hover:text-white'>login</p>
    </Link>
  <p className='text-base border-b px-2 py-1 border-gray-400 hover:text-white' style={{cursor:'pointer'}} onClick={()=>alert("User Logout Successfully")} >logout</p>
  </div>
  </NavBarMenuModal>
)
// sidebarlinkcontent
...
<button className= 'text-left text-green-300 hover:text-white'>
 ...
 <button className='text-left text-green-300 hover:text-white cursor-pointer' onClick={()=>toggleMenu("Records")}>
  <div className='flex gap-4 items-center'>
  <div className='flex flex-row  items-center gap-2 justify-center '>
 <Database size={18}/>
  <span className='text-md md:text-md lg:text-lg'>Record Section</span>
  </div>
  <div>
     <ChevronDown className={`  transition-transform delay-100 duration-300 ${openMenu === "Records" ? "rotate-180" : ""}`}
        size={18}/>
  </div>
  </div>
 </button>
{
  openMenu==='Records' && (
      ...
      <button className='cursor-pointer  hover:text-white'>
        <Link to='/dataentrycont'>
  <div className='flex flex-row  items-center items-left  gap-2'>
 <SubscriptIcon size={18}/>
  <span className='text-sm md:text-md lg:text-lg'>Subscribers</span>
  </div>
  </Link>
 </button>
  ...
  <div className='flex flex-row  items-center items-left  gap-2'>
 <SearchX size={18}/>
  <span className='text-sm md:text-md lg:text-lg'>Query Customers</span>
  </div>
  </Link>
 </button>
  ...
  <div className='flex flex-row  items-center items-left  gap-2'>
 <BookIcon size={18}/>
  <span className='text-sm md:text-md lg:text-lg'>Bookings</span>
  </div>
  </Link>
 </button>
 ...
 <LoaderPinwheelIcon size={18}/>
  <span className='text-sm md:text-md lg:text-lg'>Sign In Users</span>
  </div>
  </Link>
 </button>
 ...
 <ShoppingCart size={18}/>
  <span className='text-sm md:text-md lg:text-lg'>Buyings</span>
    ...
    <span className='text-md md:text-md lg:text-lg'>Current Story</span>
    </div>
    </Link>
    </button>
    ...
    <span className='text-md md:text-md lg:text-lg'>Complain Section</span>
    </div>
    </Link>
    </button>
    ...
    <button className='cursor-pointer text-green-300  hover:text-white'>
     ...
     <div className='flex flex-row  items-center items-left  gap-2'>
     <GitCompareIcon size={18}/>
    <span className='text-md md:text-md lg:text-lg'>Find a Movie</span>
    </div>
    </Link>
    </button>
  ...
  <nav className= "h-auto fixed top-0 w-full z-200 transition-all duration-300 bg-slate-950/80 backdrop-blur-lg border-b">
     <div className='max-w-7xl mx-auto px-4 py-2 flex justify-between items-center'>
        <div className='flex flex-row items-center gap-2 cursor-pointer'>
          ...
          <img src='/logo.png' className='w-5 h-5 sm:h-7 sm:w-7 lg:h-8 lg:w-8 hover:scale-104 delay-200 duration-200 transition-all '/>
          <p className='text-3xl sm:4xl md:5xl lg:6xl bg-linear-to-r from-blue-500/25 to-cyan-500 text-transparent bg-clip-text font-bold '>CineWave</p>
        </div>
      

        <div className='hidden md:flex space-x-4 mt-1'>
          <Link to='/' className='text-green-300 hover:text-white text-sm lg:text-xl font-semibold'  >Home</Link>
          <Link to='/about' className='text-green-300 hover:text-white text-sm lg:text-xl  font-semibold'  >About Us</Link>
          <Link to='/contact' className='text-green-300 hover:text-white text-sm lg:text-xl  font-semibold' >Contact Us</Link>
        </div>
       ...
       <div onClick={()=>setisOpen(true)} className='p-2 border border-gray-300 rounded-full bg-blue-500/20 text-blue-600
     ...
     </div>
      {
        ...
        <div className='md:hidden bg-slate-900/95 backdrop-blur-lg border-slate-800 duration-700 animate-in slide-in-from-top flex flex-col gap-2'>
          ...
          <Link to='/' className='text-green-300 hover:text-white text-sm lg:text-base' >Home</Link>
          <Link to='/about' className='text-green-300 hover:text-white text-sm lg:text-base'>About Us</Link>
          <Link to='/contact' className='text-green-300 hover:text-white text-sm lg:text-base' >Contact Us</Link>
          </div>
        </div>
          )
        }
