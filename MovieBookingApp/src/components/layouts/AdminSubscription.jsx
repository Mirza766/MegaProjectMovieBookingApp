  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-100 tracking-tight">Subscription Management</h2>
          <p className="text-slate-400 text-sm">Review, edit, or remove Subscriptions from the system.</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 px-4 py-2 rounded-lg text-sm text-slate-300">
          Total Subscriptions: <span className="text-blue-400 font-bold">{usersData.length}</span>
        </div>
      </div>
                ...
                usersData.map((curUser, index) => (
                  <tr key={index} className="hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-slate-400 whitespace-nowrap font-mono text-sm">
                      {curUser.phoneNumber || "N/A"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="flex justify-end gap-3">
                        <Link 
                          to={`/admin/subscriptions/${curUser._id}/edit`}
                        ...
                        <button 
                          onClick={() => DeleteSubscription(curUser._id)}
