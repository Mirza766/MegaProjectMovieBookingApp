...
<div className='pl-3 flex gap-2 cursor-pointer  transition-all duration-300 hover:scale-102 mb-4 sm:mb-6 lg:mb-8'>
 <span className='bg-linear-to-r from-blue-400 via-cyan-400 to-blue-400 text-transparent  bg-clip-text text-lg font-bold'>{FeedbackForm.length} Comments</span> 
 <MessageSquare className='h-7 w-7 border rounded-md text-blue-500 border-blue-500/20 ' /> 
    ...
    <div className='flex flex-col gap-5'>
      <div className='flex flex-col gap-3'>
        {
            FeedbackForm.map((data)=>(
                <div key={data.id} className='flex flex-col bg-slate-900/50  border gap-2 border-slate-800 rounded-xl sm:rounded-2xl p-2 sm:p-3 backdrop-blur-sm ' >     
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full w-fit mt-1 font-medium
                    ${data.commentType === 'Positive' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}>
                    {data.commentType}
                  </span>
                </div>
                </div>
                <p className=' font-base'>{data.comment}</p>
                </div>
            ))
        }
    </div>
