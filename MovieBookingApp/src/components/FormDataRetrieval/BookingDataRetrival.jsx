...
const {bookingDetails}=useSelector((state)=>state.MovieBooking);
    ...
    <div className='text-white px-7 grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3'>
    {
       bookingDetails.map((book)=>{
        ...
        return(
          <div key={book.id} className='relative w-full  bg-white/5 backdrop-blur-xl rounded-2xl p-2 sm:p-3  shadow-2xl hover:shadow-blue-400 transition-all delay-100 duration-300 border border-white/10 hover:-translate-y-2'>
      <div  className='w-full  text-left bg-linear-to-r from-gray-900/10 to-gray-800/10 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-2xl border border-white/10'>
        <p  className='cont-retrieve-data-para'><span className='font-semibold bg-linear-to-r from-blue-400 via-cyan-400 to-blue-400 text-transparent  bg-clip-text'>User ID: </span>{book.id}</p>
